...
Hook 能否使用取决于操作系统、微信/支付宝客户端版本、账户类型和平台规则。本文只描述项目现有适配，不保证第三方客户端升级后继续可用。生产环境优先使用官方商户渠道。
...
3. **安全提醒**：Hook 辅助端不应成为公网入口。**请勿将内嵌后端端口（默认 `62233`）暴露在公网中**；旧文档称其为 WebSocket 端口的说法不再作为 v3 部署依据。
:::
...
### (3) 因数据库写入延迟调大超时
:::warning[关键配置微调]
由于微信本地数据库持久化机制的原因，手机收到付款后，电脑端微信写入本地数据库可能存在延迟。
因此，使用 Hook v4 通道时，应根据实测延迟适当调大插件端 `config.yml` 的超时时间；可先以 200 秒作为排障起点，再根据日志和业务要求调整：
```yaml
# config.yml
payment:
  timeout: 200
