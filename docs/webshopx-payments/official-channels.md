...
官方支付通道通过各平台的商户接口完成订单预创建、扫码与状态同步。相比按金额匹配的 Hook 方案，它能按平台订单号区分并发订单，也不需要 Hook 软件常驻；生产使用前仍应完成沙盒或小额实测、回调验证和账单对账。

v3.1.1 支持从 WebShopX 的 Provider 设置页维护插件公开的渠道字段，但密钥仍由 WebShopX-Payments 校验和保存。页面未展示的高级字段继续编辑当前版本生成的 `backend/config.json`。

:::warning[升级时不要覆盖凭据]

从旧版升级时先备份 `backend/config.json`，再逐项迁移。不要用文档示例整文件覆盖生产配置，也不要把真实私钥、Token 或商户证书发到公开工单。
